...
var $db_host = 'db2653.perfora.net';     // Database Host
    var $db_user = 'dbo344252116';          // Username
    var $db_pass = 'newyork';          // Password
    var $db_name = 'db344252116';          // Database
    ...
    */
    public function select($table,$alias, $rows = '*', $where = null, $order = 'id ASC', $joins=null, $limit=null)
    {
        $q = 'SELECT '.$rows.' FROM '.$table.' AS '.$alias;
        if($joins != null){
        	  $j = ' '.$joins['type'];
        	  $j .= ' '.$joins['table'].' as '.$joins['alias'];
        	  $j .= ' ON '.$joins['conditions'];
...
//            $where = implode('',$where);
            ...
            {
                if(is_string($where[$wheres[$i]]))
                {
                    $sql_where .= $wheres[$i].'="'.$where[$wheres[$i]].'"';
                }
                else
                {
                    $sql_where .= $keys[$i].'='.$where[$wheres[$i]];
                }
