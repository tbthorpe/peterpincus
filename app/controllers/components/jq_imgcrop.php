...
class JqImgcropComponent extends Object { 
    ...
    function uploadImage($uploadedInfo, $uploadTo, $prefix){ 
		...
		$o_filename = "o_".$prefix.basename($uploadedInfo["name"]); 
		$s_filename = "s_".$prefix.basename($uploadedInfo["name"]); 
        $file_ext = substr($filename, strrpos($filename, ".") + 1); 
        $uploadTarget = $upload_path.$filename; 
		$o_uploadTarget = $upload_path.$o_filename;
		$s_uploadTarget = $upload_path.$s_filename;
        ...
        if (isset($uploadedInfo['name'])){ 
			...
			copy($uploadTarget, $o_uploadTarget );
			copy($uploadTarget, $s_uploadTarget ); 
            chmod ($uploadTarget , 0777);
			chmod ($o_uploadTarget , 0777);
			
			 //$original = $this->copyImage($o_uploadTarget,$width,$height,$scale); 
			...
			$width = $this->getWidth($s_uploadTarget); 
            $height = $this->getHeight($s_uploadTarget); 
            // Scale the image if it is greater than the width set above 
            if ($width > $s_max_width){ 
                $scale = $s_max_width/$width; 
                $uploaded = $this->resizeImage($s_uploadTarget,$width,$height,$scale); 
            }else{ 
                $scale = 1; 
                $uploaded = $this->resizeImage($s_uploadTarget,$width,$height,$scale); 
            }
...
$ext = strtolower(substr(basename($image), strrpos(basename($image), ".") + 1)); 
    ...
    function cropImage($prefix,$thumb_width, $x1, $y1, $x2, $y2, $w, $h, $thumbLocation, $imageLocation){ 
		$ext = strtolower(substr(basename($thumbLocation), strrpos(basename($thumbLocation), ".") + 1)); 
		$extLen = strlen($ext)+1;
		$filename = substr_replace($thumbLocation,"_".$prefix,strlen($thumbLocation)-$extLen,0);
				// debug($thumbLocation);
				// debug($filename);
        $scale = $thumb_width/$w; 
        $cropped = $this->resizeThumbnailImage(substr(WWW_ROOT,0,-1).str_replace("/", DS,$filename),substr(WWW_ROOT,0,-1).str_replace("/", DS,$imageLocation),$w,$h,$x1,$y1,$scale);
        return $cropped; 
    } 
} 
?>